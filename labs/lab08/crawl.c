...
int main(int argc, char **argv)
{
	...
	pushOnto(s, firstURL);
	while (!emptyStack(s) && nVertices(g) < maxURLs) {
	    strcpy(firstURL, popFrom(s));
	
	    if (!allowedURL(firstURL)) continue;
        ...
        while(!url_feof(handle)) {
            ...
            while ((pos = GetNextURL(buffer, firstURL, result, pos)) > 0) {
	            if (!allowedURL(result)) continue;
	            
	            if (nVertices(g) < maxURLs || !isConnected(g, firstURL, result)) {
                    addEdge(g, firstURL, result);
                }
	            
	            if (!isElem(visited, result)) {
	                insertInto(visited, firstURL);
	                pushOnto(s, result);
	                printf("Found: '%s'\n",result);
	            }
...
int allowedURL(char *haystack) {
    char *needle = "unsw.edu.au";
