...
BSTree BSTreeDeleteRoot2(BSTree t)
{
	...
	// Fixing BSTree links...
	if (parent == t) parent->right = NULL;
	else parent->left = NULL;

	return t;
}

BSTree BSTreeDelete2(BSTree t, int value)
{
	if (t == NULL) return NULL;
	if (t != NULL) {
		if (value < t->value) {
			t->left = BSTreeDelete2(t->left, value);
		} else if (value > t->value) {
			t->right = BSTreeDelete2(t->right, value);
		} else {
			t = BSTreeDeleteRoot2(t);
		}
	}
	return t;
}
...
void BSTreeLevelOrder(BSTree t)
{
	...
	while (!QueueIsEmpty(q)) {
	    Item l = QueueLeave(q);
	    printf("%d ", l->value);
	    if (l->left != NULL) QueueJoin(q, l->left);
	    if (l->right != NULL) QueueJoin(q, l->right);
    }
}
