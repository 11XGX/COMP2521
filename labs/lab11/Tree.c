...
Link insertSplay(Link t, Item it)
{
	...
	if (diff == 0)
		...
		if (incComp(t) || less(v,key(t->left->value))) {
		...
		if (incComp(t) || less(key(t->right->value),v)) {
...
static Link searchSplay(Link t, Key k, int *found)
{
	...
	else if (incComp(t) || eq(key(t->value),k)) {
		*found = 1; // item found, store true  
		res =  t;  
	}
	else if (incComp(t) || less(k,key(t->value))) {
		if (t->left == NULL){
			*found = 0;// item not found
			//res = rotateRight(t); 
			res = t;
		}
		else if (incComp(t) || eq(key(t->left->value),k)) {
			*found = 1;
			res = rotateR(t);
		}
		else {
			if (incComp(t) || less(k,key(t->left->value))) {
				// left-left
		...
		else if (incComp(t) || eq(key(t->right->value),k)) {
			*found = 1;
			res = rotateL(t);
		}
		else{
			if (incComp(t) || less(key(t->right->value),k)) {
				/* right-right */
