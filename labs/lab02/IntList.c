...
static void IntNodeInsertInOrder(struct IntListNode *curr, int v);
...
void IntListInsertInOrder(IntList L, int v)
{
	...
	if (L->first == NULL) {
		L->first = L->last = newIntListNode(v);
	} else if (v <= L->first->data) {
		struct IntListNode *newNode = newIntListNode(v);
		newNode->next = L->first;
		L->first = newNode;
...
static void IntNodeInsertInOrder(struct IntListNode *curr, int v) {
	...
	IntNodeInsertInOrder(curr->next, v);
}
