...
int connections(Map g, LocationID start, LocationID end, TransportID type[])
{
   ...
   if (g->connections[start] == NULL || g->connections[end] == NULL) {
      return 0;
   }
   
   int i = 0;
   VList curr = g->connections[start];
   VList node_end = g->connections[end];
   
   while (curr != NULL) {
      if (curr->v == end && (isLand(start) || (isSea(start) && isSea(end))) {
         type[i] = curr->type;
         i++;
      }
      if (curr->type == BOAT) {
         while (node_end != NULL) {
            if (curr->v == node_end->v) {
                type[i] = curr->type;
                i++;
            }
