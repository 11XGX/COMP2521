...
{
   int min = MIN_MAP_LOCATION;
   int max = MAX_MAP_LOCATION;
   int mid, result;
   
   while (min <= max) {
      // Find the index to be compared.
      mid = (min+max)/2;
      // Comparison...
      result = strcmp(name, places[mid].name);
      // If less than mid, set new max.
      if (result < 0) max = mid;
      // If more than mid, set new min.
      else if (result > 0) min = mid;
      // Else, return mid's id.
      else return places[mid].id;
   }
   return NOWHERE;
}
