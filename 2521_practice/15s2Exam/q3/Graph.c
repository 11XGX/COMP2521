...
static int visitedCheck(Graph g, Vertex v, int d, int *visited)
{
   if (d == 0) return 0;
   visited[v] = 1;
   Vertex w;
   for (w = 0; w < g->nV; w++) {
      if (g->edges[v][w] && !visited[w]) {
         return 1 + visitedCheck(g, w, d-1, visited);
      }
   }
   return 0 + visitedCheck(g, w, d, visited);
}
...
int within(Graph g, Vertex s, int d, Vertex *vs)
{
   ...
   for (i = 0; i < g->nV; i++) visited[i] = 0;
   int num = visitedCheck(g, s, d, visited);
   int j = 0;
   for (i = 0; i < g->nV; i++) {
      if (visited[i]) vs[j++] = i;
   }
